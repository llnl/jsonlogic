...
CXX_NORETURN
inline void throw_type_error() { throw type_error("typing error"); }

CXX_NORETURN
inline void implementation_error() {
...
static_assert(
    std::is_same_v<
        bool, std::variant_alternative_t<bool_variant, value_variant_base>>);
static_assert(std::is_same_v<
              std::int64_t,
...
static_assert(
    std::is_same_v<
        double, std::variant_alternative_t<real_variant, value_variant_base>>);
static_assert(std::is_same_v<
              managed_string_view,
...
template <class T>
bool stricteq(const value_variant &lhs, const value_variant &rhs) try {
...
using variant_span_base =
    std::tuple<std::vector<value_variant>::const_iterator,
               std::vector<value_variant>::const_iterator>;

struct variant_span : variant_span_base {
  using base = variant_span_base;
  using base::base;

  std::vector<value_variant>::const_iterator begin() const {
    return std::get<0>(*this);
  }
  std::vector<value_variant>::const_iterator end() const {
    return std::get<1>(*this);
  }
  std::size_t size() const { return std::distance(begin(), end()); }
};

managed_string_view element_range(const value_variant &val,
                                  const std::string_view &) {
  return std::get<managed_string_view>(val);
}

variant_span element_range(const std::vector<value_variant> &vec) {
  return {vec.begin(), vec.end()};
}

variant_span element_range(const array_value *arr) {
  return element_range(deref(arr).value());
}

variant_span element_range(const value_variant &val, const array_value *) {
  return element_range(std::get<array_value const *>(val));
}

template <class RangeT>
bool stricteq_range(const value_variant &lhs, const value_variant &rhs) {
...
bool inteq(std::int64_t x, std::uint64_t y) {
  bool const y_within_int_range =
      (y <= std::uint64_t(std::numeric_limits<std::int64_t>::max()));

  return y_within_int_range & (x == std::int64_t(y));
}

template <class T>
bool inteq(const value_variant &lhs, const value_variant &rhs) {
  if (rhs.index() == lhs.index()) {
    CXX_LIKELY;
    return std::get<T>(lhs) == std::get<T>(rhs);
  }
...
namespace std {
size_t hash<jsonlogic::array_value>::operator()(
    const jsonlogic::array_value &v) const {
  auto values =
      v.value() | std::views::transform(std::hash<jsonlogic::value_variant>{});

  // C++23: return std::ranges::fold_left(values, size_t(0),
  // std::plus<size_t>{});
  return std::accumulate(values.begin(), values.end(), size_t(0),
                         std::plus<size_t>{});
}

size_t hash<jsonlogic::value_variant_base>::operator()(
    const jsonlogic::value_variant_base &v) const {
  ...
  case jsonlogic::strv_variant:
    value_hash =
        std::hash<string_view>{}(std::get<jsonlogic::managed_string_view>(v));
    break;

  case jsonlogic::sequ_variant:
    value_hash = std::hash<jsonlogic::array_value>{}(
        *std::get<jsonlogic::array_value const *>(v));
    break;
...
size_t hash<jsonlogic::value_variant>::operator()(
    const jsonlogic::value_variant &v) const {
...
namespace jsonlogic {
namespace {
/// functor testing if a jsonlogic expression can be converted to a variant
/// representation.
struct convertible_to_value_variant {
  ...
  auto operator()(const T &) const
      -> decltype(std::declval<T>().to_variant(), bool{}) {
    return true;
  }
};
...
/// \details
///   the functor throws an std::logic_error if the conversion cannot be
///   performed.
struct value_variant_conversion {
  ...
  auto operator()(const T &o) const
      -> decltype(std::declval<T>().to_variant(), value_variant{}) const {
    return o.to_variant();
  }
};
...
std::unordered_set<value_variant> try_static_set(const any_expr &e) {
  ...
  if (auto pos = std::find_if(beg, lim, not_convertible_to_value_variant);
      pos != lim)
    return {};
...
value_variant::~value_variant() { delete_array(*this); }

value_variant::value_variant(const value_variant &other) : base(other) {
  assert(this != &other);
  copy_array(*this);
}

value_variant::value_variant(value_variant &&other) : base(std::move(other)) {
  assert(this != &other);
...
array_value::container_type const &array_value::value() const {
  assert(vec.get());

  return *vec;
}

const array_value *array_value::copy() const { return new array_value(*this); }

value_variant array_value::to_variant() const { return this; }

#if ENABLE_OPTIMIZATIONS
void opt_membership_array::set_elems(std::unordered_set<value_variant> els) {
  elements = std::move(els);
}

std::unordered_set<value_variant> const &opt_membership_array::elems() const {
  return elements;
}
...
template <class T> value_variant value_generic<T>::to_variant() const {
  return value();
}
...
#if ENABLE_OPTIMIZATIONS
  // optimizations
  void visit(const opt_membership_array &n) override {
    visit(up_cast<oper>(n));
  }
#endif /* ENABLE_OPTIMIZATIONS */
};
...
template <class ExprT> ExprT &mk_operator_(oper::container_type args) {
...
#if ENABLE_OPTIMIZATIONS
  if (std::unordered_set<value_variant> elems = try_static_set(args.back());
      elems.size() != 0) {
    args.pop_back();
    opt_membership_array &res =
        mk_operator_<opt_membership_array>(std::move(args));
...
oper::container_type translate_children(const json::value &n,
                                        variable_map &varmap) {
...
logic_rule create_logic(const json::value &n) {
  ...
  return logic_rule(std::make_unique<logic_data>(
      std::move(node), varmap.to_vector(), hasComputedVariables));
}
...
any_value
to_value(const json::value &n); // \todo remove after moving to logic.hpp
...
struct numeric_binary_operator_base {
  ...
  std::tuple<double, double> coerce(const double *lv,
  ...
  std::tuple<double, double> coerce(const std::int64_t *lv,
                                    const double *rv) const {
  ...
  std::tuple<double, double> coerce(const std::uint64_t *lv,
                                    const double *rv) const {
    return {to_concrete(*lv, *rv), *rv};
  }
  ...
  coerce(const std::uint64_t *lv, const std::uint64_t *rv) const {
    return {*lv, *rv};
  }
};
...
struct relational_operator_base : numeric_binary_operator_base {
  ...
  coerce(const std::int64_t *lv, const managed_string_view *rv) const {
    return {*lv, to_concrete(*rv, *lv)};
  }
  ...
  std::tuple<std::uint64_t, std::uint64_t>
  coerce(const std::uint64_t *lv, const managed_string_view *rv) const {
    return {*lv, to_concrete(*rv, *lv)};
  }
  ...
  std::tuple<double, double> coerce(const managed_string_view *lv,
                                    const double *rv) const {
    return {to_concrete(*lv, *rv), *rv};
  }
  ...
  std::tuple<std::uint64_t, std::uint64_t>
  coerce(const managed_string_view *lv, const std::uint64_t *rv) const {
    return {to_concrete(*lv, *rv), *rv};
  }

  std::tuple<std::uint64_t, std::uint64_t>
  coerce(const bool *lv, const std::uint64_t *rv) const {
    return {to_concrete(*lv, *rv), *rv};
  }

  std::tuple<bool, bool> coerce(const managed_string_view *,
                                const bool *) const {
    // strings and boolean are never equal
    return {true, false};
  }

  std::tuple<bool, bool> coerce(const bool *,
                                const managed_string_view *) const {
    // strings and boolean are never equal
    return {true, false};
  }

  std::tuple<const managed_string_view &, const managed_string_view &>
  coerce(const managed_string_view *lv, const managed_string_view *rv) const {
    return {*lv, *rv};
  }
...
struct equality_operator : relational_operator_base, comparison_operator_base {
  ...
  std::tuple<bool, bool> coerce(const array_value *,
                                const array_value *) const {
...
struct relational_operator : relational_operator_base,
  ...
  std::tuple<variant_span, variant_span> coerce(const array_value *lv,
                                                const array_value *rv) const {
  ...
  std::tuple<const managed_string_view &, std::nullptr_t>
  coerce(const managed_string_view *lv, std::nullptr_t) const {
  ...
  std::tuple<std::uint64_t, std::uint64_t>
  coerce(std::nullptr_t, const std::uint64_t *rv) const {
    return {0, *rv}; // null pointer -> 0
  }
  ...
  std::tuple<std::nullptr_t, const managed_string_view &>
  coerce(std::nullptr_t, const managed_string_view *rv) const {
...
struct arithmetic_operator : numeric_binary_operator_base {
  ...
  coerce(std::nullptr_t, const std::int64_t *) const {
    return {nullptr, nullptr};
  }

  std::tuple<std::nullptr_t, std::nullptr_t>
  coerce(std::nullptr_t, const std::uint64_t *) const {
    return {nullptr, nullptr};
  }
...
struct arithmetic_converter {
  template <class T> CXX_NORETURN any_value operator()(T) const {
    throw_type_error();
  }
...
struct string_converter {
  template <class T> CXX_NORETURN managed_string_view operator()(T) const {
    throw_type_error();
  }
  ...
  // need to convert values
  managed_string_view operator()(bool val) const {
    return to_concrete(val, std::string_view{});
  }
  managed_string_view operator()(std::int64_t val) const {
    return to_concrete(val, std::string_view{});
  }
  managed_string_view operator()(std::uint64_t val) const {
    return to_concrete(val, std::string_view{});
  }
  managed_string_view operator()(double val) const {
    return to_concrete(val, std::string_view{});
  }
  managed_string_view operator()(std::nullptr_t) const {
    return to_concrete(nullptr, std::string_view{});
  }
};

managed_string_view convert(any_value val,
                            const string_operator_non_destructive &) {
...
struct array_converter {
  ...
  // defined for the following types
  template <class T> CXX_NORETURN result_type operator()(T &&) const {
    throw_type_error();
  }

  result_type operator()(array_value const *v) const { return v->copy(); }

  // need to move value_base to new array
  result_type operator()(const managed_string_view &v) const {
    return to_array(to_value(v));
  }
...
template <class value_t> struct tag_type_traits {
  using type = value_t;
};

template <> struct tag_type_traits<managed_string_view> {
  using type = std::string_view;
};

template <class value_t> struct unpacker {
  using tag_type = tag_type_traits<value_t>::type;

  value_t conv(const value_t &val, const tag_type &) { return val; }

  template <class U> value_t conv(const U &val, const tag_type &tag) const {
    return to_concrete(val, tag);
  }

  template <class T> CXX_NORETURN value_t operator()(T) const {
    throw_type_error();
  }

  // defined for the following types
  value_t operator()(const managed_string_view &val) const {
...
bool truthy(const any_value &el) { return unpack_value<bool>(el); }
...
struct binary_operator_visitor_2 { // : forwarding_visitor {
  ...
  template <class rhs_value_t> result_type calc(rhs_value_t rv) const {
...
template <class binary_op_t> struct binary_operator_visitor {
  using result_type = typename binary_op_t::result_type;

  binary_operator_visitor(binary_op_t oper, const any_value &rhsarg)
      : op(oper), rhs(rhsarg) {}

  template <class LhsValue> result_type calc(LhsValue lv) const {
...
compute(const any_value &lhs, const any_value &rhs, binary_op_t op) {
...
bool compare_sequence(variant_span lv, variant_span rv,
  ...
  bool res = false;
  auto cmpElems = [&pred, &res](const any_value &lhs,
                                const any_value &rhs) -> bool {
    res = compute(lhs, rhs, pred);
    return res == compute(rhs, lhs, pred);
  };

  auto const [lpos, rpos] =
      std::mismatch(lbeg, lbeg + len, rv.begin(), cmpElems);
...
template <class binary_predicate_t>
bool compare_sequence(array_value const *lv, array_value const *rv,
                      binary_predicate_t pred) {
  return compare_sequence(element_range(lv), element_range(rv),
                          std::move(pred));
}
...
/// \{
template <class U, class V, class R> struct mismatched_types {
  using type = R;
};

template <class T, class R> struct mismatched_types<T, T, R> {
...
template <> struct operator_impl<cat> : string_operator_non_destructive {
  ...
  result_type operator()(const managed_string_view &lhs,
                         const managed_string_view &rhs) const {
    std::string tmp;
...
template <> struct operator_impl<membership> : string_operator_non_destructive {
  using string_operator_non_destructive::result_type;
...
struct sequence_function {
  ...
  any_value operator()(const any_value &elem) const {
    evaluator sub{[&elem](value_variant keyval, int) -> any_value {
                    if (managed_string_view *pkey =
                            std::get_if<managed_string_view>(&keyval)) {
                      if (pkey->size() == 0)
                        return elem;
                    }
...
struct sequence_reduction {
  ...
  any_value operator()(any_value accu, any_value elem) const {
    evaluator sub{[&accu, &elem](value_variant keyval, int) -> any_value {
                    if (const managed_string_view *pkey =
                            std::get_if<managed_string_view>(&keyval)) {
                      CXX_LIKELY;
                      if (*pkey == "current")
                        return elem;
...
void evaluator::visit(const substr &n) {
  ...
  managed_string_view str =
      unpack_value<managed_string_view>(eval(n.operand(0)));
...
void evaluator::visit(const array &n) {
  ...
  evaluator *self = this;
  auto eval_array_elems = [self](const any_expr &exp) -> any_value {
    return self->eval(*exp);
  };

  std::transform(n.begin(), n.end(), std::back_inserter(elems),
                 eval_array_elems);
...
void evaluator::visit(const reduce &n) {
  ...
  auto op = [&expr, accu,
             calclogger = &this->logger](array_value const *v) -> any_value {
    variant_span spn = element_range(v);
    return std::accumulate(spn.begin(), spn.end(), std::move(accu),
...
void evaluator::visit(const map &n) {
  ...
  auto mapper = [&n, &arr, calclogger = &this->logger](
    ...
    std::transform(spn.begin(), spn.end(), std::back_inserter(mapped_elements),
...
void evaluator::visit(const filter &n) {
  ...
  auto filter = [&n, &arr, calclogger = &this->logger](
    ...
    std::copy_if(spn.begin(), spn.end(), std::back_inserter(filtered_elements),
...
evaluator::missing_aux(const oper &n, std::size_t arrpos) {
  ...
  auto non_array_alt = [&tmpval, &arr, &n, pos = arrpos + 1,
                        calc = this]() -> variant_span {
    std::transform(
        std::next(n.begin(), pos), n.end(), std::back_inserter(tmpval),
  ...
  auto unwrapped = [](array_value const *v) -> variant_span {
    return element_range(v);
  };

  variant_span elems =
      with_type<array_value const *>(arr, unwrapped, non_array_alt);
  ...
  std::copy_if(std::make_move_iterator(elems.begin()),
               std::make_move_iterator(elems.end()), std::back_inserter(res),
               notavail);

  return {std::move(res), elems.size() - res.size()};
}
...
template <class IntT> any_value eval_index(IntT idx, const json::array &arr) {
  return jsonlogic::to_value(arr[idx]);
}

any_value apply(const expr &exp, const variable_accessor &vars) {
  variant_logger logger = [](const value_variant &val) -> void {
...
any_value apply(logic_data &rule) {
  auto no_variables = [](value_variant, int) -> any_value {
...
variable_accessor json_accessor(json::value data) {
  return [data = std::move(data)](value_variant keyval, int) -> any_value {
    if (const managed_string_view *ppath =
            std::get_if<managed_string_view>(&keyval)) {
